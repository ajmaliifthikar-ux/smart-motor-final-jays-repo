...
### 2026-02-21: Hardcoded API Keys in Development Endpoints
- **Vulnerability:** Hardcoded Gemini API keys were present as fallbacks in diagnostics and admin routes.
- **Learning:** Developers often hardcode valid keys as fallbacks during testing or rapid development and forget to remove them before committing. This leaks credentials to source control.
- **Prevention:** Always use an empty string fallback (`process.env.GEMINI_API_KEY || ''`) to satisfy TypeScript's string requirements without introducing vulnerabilities. This securely triggers a runtime error if the key is missing in the environment, enforcing proper configuration.
