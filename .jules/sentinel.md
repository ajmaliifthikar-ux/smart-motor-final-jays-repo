...
## 2026-02-16 - Remove Hardcoded Gemini API Key
**Vulnerability:** A hardcoded Gemini API key was exposed in multiple source files and documentation as a fallback.
**Learning:** Fallbacks for missing environment variables are sometimes written using hardcoded keys during early development, which easily leak into production code.
**Prevention:** Always validate sensitive environment variables explicitly and throw errors if they are missing (e.g., `if (!apiKey) throw new Error(...)`) instead of providing default hardcoded values.
