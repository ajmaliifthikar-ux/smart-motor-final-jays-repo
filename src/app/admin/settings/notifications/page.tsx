  ...
  const runDailyReportNow = async () => {
    ...
    try {
      const res = await fetch('/api/cron/daily-report', {
        headers: {} // Authorization happens via admin session cookie automatically
      })
      const data = await res.json()
      if (data.success) {
      ...
      <div className="bg-gray-900 rounded-2xl p-6 text-white">
        ...
        <ol className="space-y-3 text-sm text-gray-300">
          {[
            ...
            <>Add <code className="bg-gray-800 px-1.5 py-0.5 rounded text-xs text-green-400">CRON_SECRET=your-secret</code> to Vercel securely (to authenticate Vercel Cron execution)</>,
