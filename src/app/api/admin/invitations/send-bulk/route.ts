...
export async function POST(request: NextRequest) {
    try {
        ...
        // Fail securely if secretKey is not configured in environment
        const hasValidSecretAuth = secretKey ? authHeader === `Bearer ${secretKey}` : false
        const isAuthorized = hasValidSecretAuth || session?.user?.id
