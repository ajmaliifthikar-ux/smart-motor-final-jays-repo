...
const globalGenAI = process.env.GEMINI_API_KEY ? new GoogleGenerativeAI(process.env.GEMINI_API_KEY) : null;
...
export class AIMemoryManager {
    ...
    async generateEmbedding(text: string): Promise<number[]> {
        try {
            if (!globalGenAI) throw new Error('GEMINI_API_KEY is missing'); const model = globalGenAI.getGenerativeModel({ model: 'text-embedding-004' })
...
`
        ...
        try {
            ...
            const apiKey = process.env.GEMINI_API_KEY; if (!apiKey) throw new Error('GEMINI_API_KEY is missing'); const genAI = new GoogleGenerativeAI(apiKey);
