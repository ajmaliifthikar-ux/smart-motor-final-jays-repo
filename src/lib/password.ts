...
export function generateRandomPassword(): string {
  ...
  // Allocate single array for all random operations
  const randomValues = new Uint32Array(12 + 12) // enough for password gen + shuffle
  globalThis.crypto.getRandomValues(randomValues)
  let randomIdx = 0

  // Ensure all character types are included
  password += uppercase.charAt(Math.floor((randomValues[randomIdx++] / 0x100000000) * uppercase.length))
  password += lowercase.charAt(Math.floor((randomValues[randomIdx++] / 0x100000000) * lowercase.length))
  password += numbers.charAt(Math.floor((randomValues[randomIdx++] / 0x100000000) * numbers.length))
  password += special.charAt(Math.floor((randomValues[randomIdx++] / 0x100000000) * special.length))

  // Fill rest with random characters from all types
  const allChars = uppercase + lowercase + numbers + special
  for (let i = password.length; i < 12; i++) {
    password += allChars.charAt(Math.floor((randomValues[randomIdx++] / 0x100000000) * allChars.length))
  }

  // Shuffle password (secure Fisher-Yates)
  const passwordArray = password.split('')
  for (let i = passwordArray.length - 1; i > 0; i--) {
    const j = Math.floor((randomValues[randomIdx++] / 0x100000000) * (i + 1))
