...
export function generateShortCode(
  ...
  const totalChars = segments * charactersPerSegment
  const randomBuffer = new Uint32Array(totalChars)
  ...
  for (let i = 0; i < segments; i++) {
    ...
    for (let j = 0; j < charactersPerSegment; j++) {
      segment += chars.charAt(randomBuffer[bufferIndex++] % chars.length)
    }
    codes.push(segment)
  }
...
export function generateBackupCodeId(): string {
  ...
  const randomBuffer = new Uint32Array(8)
  globalThis.crypto.getRandomValues(randomBuffer)

  for (let i = 0; i < 8; i++) {
    code += chars.charAt(randomBuffer[i] % chars.length)
  }
  return code
}
